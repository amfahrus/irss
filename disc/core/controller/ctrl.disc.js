...
define([
    ...
    function Ctrldisc($scope,$window,$sce,serviceAjax){
        ...
        $scope.taketest = function() { 
			...
			$scope.halaman ='dua';
			$scope.loading = true;
			serviceAjax.posDataToServer('disc_api','takedisc',$scope.param).then(function(data){
				if (data) {
					$scope.loading = false;
					$scope.question = data;
		...
		$scope.next = function() {
			...
			serviceAjax.getDataFromServer('disc_api','get_user_disc_data',$scope.page)
				.then(function(data){
					if (data) {
						$scope.loading = false;
						$scope.question = data;
		...
		$scope.rateFunction = function(option,marks) {
			...
			if($scope.param){
				...
				serviceAjax.posDataToServer('disc_api','save_answer_disc',$scope.param).then(function(data){
					if (data) {
						$scope.loading = false;
						//$scope.$apply();
					}
		...
		$scope.finish = function() {
			$scope.halaman ='tiga';
			$scope.loading = true;     
			serviceAjax.getDataFromServer('disc_api','finish_user_disc')
				.then(function(resp){
					if (resp) {
						$scope.loading = false;
						$scope.result = resp;
...
tooltipTemplate: "<%= value %>",
		...
		$scope.email = function() { 
			serviceAjax.posDataToServer('disc_api','send_email',$scope.param).then(function(data){
				if (data) {
					//alert(data);
if(data=='success'){
