...
class Master_account extends CI_Controller {
    public function  __construct() {
        ...
        $this->load->model("m_master_account");
    }
    ...
    public function lists() {
		...
		$data['home'] = base_url().'master_account';
		$data['modul'] = 'Management Company Account';
		$data['title'] = 'List Company Account';
		$data['subtitle'] = 'Company Account';
		$data['listener'] = 'master_account/listener';
		
		$data["menu"] = $this->dokumen_lib->build_menu();
		$data['content'] = $this->load->view('backend/master_account/list', $data, TRUE);
    ...
    public function listener() {
			...
			$sumcols = 6;
			$sql = $this->m_master_account->Listener($start, $rows, $where, $this->dokumen_lib->get_sort($this->input->post('iSortCol_0'),$cols,$sumcols), $sort_dir);
			$iFilteredTotal = $sql->num_rows();

			$iTotal = $this->m_master_account->RowsAll($where)->num_rows();
    ...
    public function disable($ca_id){
        ...
        $this->m_master_account->update(array("company_account_id" => $ca_id),$data);
	}
