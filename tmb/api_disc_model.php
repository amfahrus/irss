...
class Api_disc_model extends CI_Model 
{
...
/* DISC */
	...
	function get_disc_results($questid)
    {
		...
		{			
			...
			$users_results[$key]['avg_D'] = round(($users_results[$key]['D'] * 100) / ($this->get_disc_total('D')*3), 2);
			$users_results[$key]['avg_I'] = round(($users_results[$key]['I'] * 100) / ($this->get_disc_total('I')*3), 2);
			$users_results[$key]['avg_S'] = round(($users_results[$key]['S'] * 100) / ($this->get_disc_total('S')*3), 2);
			$users_results[$key]['avg_C'] = round(($users_results[$key]['C'] * 100) / ($this->get_disc_total('C')*3), 2);
